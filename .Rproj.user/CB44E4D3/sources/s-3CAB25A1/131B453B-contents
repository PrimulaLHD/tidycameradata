tidycamera <- function(data,gridID,cameraID,year,species,date_time,End_Date,Start_Date,interval=30){
  ...
  na.camera.data <- datar %>%
    filter(is.na(date_time)) %>%
    mutate(timediff=NA,
           idependent=TRUE)
  res1 <- rbind(res1,na.camera.data) # keep Nas in your data
